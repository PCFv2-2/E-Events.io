...
function eventIdVote($eventId){
    try {
        ...
        $seasonDate = $dbMain->selectQueryAndFetch('SELECT DATE_START,DATE_END FROM `SEASONS` WHERE SEASON_ID = (SELECT MAX(SEASON_ID) FROM `SEASONS`)')[0];
        $seasonId = $dbMain->selectQueryAndFetch('SELECT SEASON_ID FROM `SEASONS` WHERE SEASON_ID = (SELECT MAX(SEASON_ID) FROM `SEASONS`)')[0][0];


        if ($currentDate >= $seasonDate[0] && $currentDate <= $seasonDate[1]){
            if (!is_null($eventId)){
